...
const PixelButton = class {
    ...
    render() {
        return (index.h(index.Host, { tabindex: "-1", class: {
            ...
            }, style: this.computedStyle }, index.h("div", { tabindex: "-1", class: "pixel-wrapper" }), index.h("button", { class: {
                'icon': this.hasPixelIcon
            }, autoFocus: this.autofocus, tabIndex: (this.disabled) ? -1 : 0, onClick: (event) => this.onClick(event), onFocus: (event) => this.onFocus(event), onBlur: (event) => this.onBlur(event), onMouseOver: (event) => this.onMouseOver(event), onMouseOut: (event) => this.onMouseOut(event) }, index.h("slot", null))));
    }
